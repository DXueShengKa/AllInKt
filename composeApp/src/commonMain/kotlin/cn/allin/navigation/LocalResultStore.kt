...
fun rememberResultStore(): ResultStore =
    rememberSaveable(saver = ResultStoreSaver()) {
        ResultStore()
    }
...
class ResultStore {
     ...
     */
    val resultStateMap: MutableMap<String, MutableState<Any?>> = hashMapOf()

    /**
     * Retrieves the current result of the given resultKey.
     */
    inline fun <reified T> getResultState(resultKey: String = T::class.toString()) = resultStateMap[resultKey]?.value as T
