...
class ChineseDate {
    ...
    constructor(localDate: LocalDate) {
        ...
        var daysOfYear: Int
        var iYear: Int = Lunar.BASE_YEAR
        while (iYear <= Lunar.MAX_YEAR) {
        ...
        chineseYear = iYear
        // 计算农历月份
        val leapMonth = Lunar.leapMonth(iYear) // 闰哪个月,1-12
        ...
        while (month < 13) {
            ...
            if (leapMonth > 0 && month == leapMonth + 1) {
                daysOfMonth = Lunar.leapDays(chineseYear)
                hasLeapMonth = true
            } else {
                // 普通月，当前面的月份存在闰月时，普通月份要-1，递补闰月的数字
                // 如2月是闰月，此时3月实际是第四个月
                daysOfMonth = Lunar.monthDays(chineseYear, if (hasLeapMonth) month - 1 else month)
            }
    ...
    fun getLunarFestivals(): String? {
        return Lunar.getFestivals(chineseYear, month, day)
    }
    ...
    fun getSolarFestivals(): String? {
        return Lunar.getSolarFestivals(gregorianMonthBase1, gregorianDay)
    }
    ...
    fun getCyclical(): String {
        return Lunar.getGanzhiOfYear(chineseYear)
    }
    ...
    fun getCyclicalYMD(): String? {
        return if (gregorianYear >= Lunar.BASE_YEAR && gregorianMonthBase1 > 0 && gregorianDay > 0) {
     ...
     */
    fun getTerm(): String {
        return Lunar.solarGetTerm(gregorianYear, gregorianMonthBase1, gregorianDay)
    }
...
//    }
    ...
    private fun cyclicalm(year: Int, month: Int, day: Int): String {
        return Lunar.getGanzhiOfYear(chineseYear) + "年" +
                Lunar.getGanzhiOfMonth(year, month, day) + "月" +
                Lunar.getGanzhiOfDay(year, month, day) + "日"
